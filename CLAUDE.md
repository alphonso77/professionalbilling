...
- **Refunds:** `charge.refunded` webhook is handled by `src/workers/stripe-events.ts::handleChargeRefunded`. Invoice is looked up by `charge.payment_intent` → `stripe_payment_intent_id`. Each Stripe Refund (`charge.refunds.data[*]`) is inserted into `invoice_refunds` (`stripe_refund_id UNIQUE` + `ON CONFLICT … IGNORE` for event-retry idempotency; Stripe fires one event per refund but the full refunds array is attached each time). If `charge.refunded === true && amount_refunded >= amount`, invoice `status` flips to `'refunded'`; partial refunds keep `status='paid'` and the detail view renders the per-refund history. Workers bypass RLS (raw `db`); the `invoice_refunds` RLS policy is the same column-check pattern as `invoice_line_items`. Read path: `getInvoiceWithItems` loads refunds alongside line items; `GET /api/invoices/:id` returns them under `refunds` in the payload. `POST /api/invoices/:id/void` refuses (409) when status is `refunded`.
