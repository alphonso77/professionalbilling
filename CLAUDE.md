...
**`PROFESSIONALBILLING_APP_PASSWORD` alphabet constraint:** the password must be URL-safe (no `@`, `:`, `/`, `?`, `#`, `%`, no whitespace) because it's interpolated into `DATABASE_APP_URL`, AND single-quote-safe for the `ALTER ROLE … PASSWORD %L` path the migration + startup sync both use. Hex (64 chars from `openssl rand -hex 32`) is the canonical choice. Special characters parse silently wrong in either path → the role ends up with a password the URL can't actually produce, connections fall back to whatever Railway interpolation resolves to, and RLS silently stops applying. No runtime validator — the guardrail is H1/H2 above plus this note.
