...
The marketing site (`alphonso77/fratellisoftware-com`, `server/subscribe.js`) fires `POST /api/webhooks/fratelli-signup` after a paid checkout. Body is `{ event: 'signup.completed', email, stripeCustomerId, stripeSubscriptionId, trialEndAt, termsAccepted, termsAcceptedAt, termsVersion, termsAcceptedIp, occurredAt }`. The `terms*` bundle is the T&C acceptance snapshot captured server-side on the marketing site when the required checkbox was ticked: `termsAcceptedAt` is a required ISO-8601 string (the marketing site won't publish signups that fail the checkbox); `termsVersion` / `termsAcceptedIp` are nullable; `termsAccepted` boolean is accepted for explicitness but not persisted (non-null `terms_accepted_at` conveys the same signal). HMAC-SHA256 signed over the raw body with `PB_WEBHOOK_SECRET`; header `X-Fratelli-Signature: sha256=<hex>`. Route mounted with `express.raw(...)` before `express.json()` (same pattern as Stripe).

Flow: verify signature (`crypto.timingSafeEqual`) → dedup on `audit_log { source: 'fratelli.signup', external_id: stripeSubscriptionId, status: 'processed' }` → `provisionCustomer()` (`src/services/clerk-provisioning.ts`) calls `clerkClient.users.createUser` + `organizations.createOrganization`, stashing `{ stripeCustomerId, stripeSubscriptionId, trialEndAt, termsAcceptedAt, termsVersion, termsAcceptedIp, source: 'fratellisoftware-com' }` on the org's `publicMetadata` → enqueue `welcome-email` (Resend, 6-digit-OTP activation link to `${FRONTEND_URL}/activate?email=…`) → 200. Clerk's async `organization.created` webhook fires a second or two later; the existing `handleClerkEvent` handler reads `publicMetadata` and writes the signup columns on `organizations` (`stripe_customer_id`, `stripe_subscription_id`, `trial_end_at`, `signup_source`, `terms_accepted_at`, `terms_version`, `terms_accepted_ip`). Replay safety: `organizations.stripe_subscription_id` is UNIQUE, and `provisionCustomer` short-circuits when the email already exists (attaching Stripe data to the user's first existing org).
